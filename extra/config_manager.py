...
from extra.models.baseline import BaseLineModel
from extra.models.cbhg import CBHGModel
from extra.models.seq2seq import Decoder as Seq2SeqDecoder, Encoder as Seq2SeqEncoder, Seq2Seq
from extra.models.tacotron_based import (
    Decoder as TacotronDecoder,
    Encoder as TacotronEncoder,
    Tacotron,
)

from extra.options import AttentionType, LossType, OptimizerType
from extra.util.text_encoders import (
...
class ConfigManager:
    ...
    def __init__(self, config_path: str, model_kind: str):
        ...
        self.data_dir = Path(
            os.path.join(self.config["data_directory"],
                         self.config["data_type"])
        )
        self.base_dir = Path(
        ...
        self.config["len_target_symbols"] = len(
            self.text_encoder.target_symbols)
    ...
    def create_remove_dirs(
        ...
        if clear_dir:
            delete = input(
                f"Delete {self.log_dir} AND {self.models_dir}? (y/[n])")
            if delete == "y":
    ...
    def get_text_encoder(self):
        ...
        if self.config["text_cleaner"] not in [
            ...
            raise Exception(
                f"cleaner is not known {self.config['text_cleaner']}")
    ...
    def get_loss_type(self):
        try:
            ...
            raise Exception(
                f"The loss type is not correct {self.config['loss_type']}")
        return loss_type
